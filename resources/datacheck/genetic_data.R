...
bim_file <- as.character("/user/work/er20212/DEEP_mqtls/input_data/data_filtered.bim")

fam_file <- as.character(args[2])
fam_file <- as.character("/user/work/er20212/DEEP_mqtls/input_data/data_filtered.fam")

quality_file <- as.character(args[3])

controlsnps_file <- as.character(args[4])
controlsnps_file <- as.character("/user/work/er20212/DEEP_mqtls/resources/genetics/snpsforbuildandposcheck.txt")
...
if (is.na(build) | build == 36) {
    msg <- paste0("Please check that the bim file is in build 37 or 38. The bim file does not appear to be in any of these builds. \n",
...
if (build != ori_build) {
    msg <- paste0("Please check that imputed genome build is GRCh", ori_build, ", but as the bim file is GRCh", build)
    errorlist <- c(errorlist, msg)
    warning("ERROR: ", msg)
}
