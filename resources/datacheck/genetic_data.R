...
miss_liftover <- as.character(args[10])
...
if (build == 37) {
    ...
    write.table(bim[!(bim$V2%in%temp_bim$V2),], file = miss_liftover, sep = "\t", quote = F, row.names = F, col.names = F)
...
message("Checking build against control SNPs by checking positions of common control SNPs")
no.SNPs.bychr <- NULL
for (i in 1:22)
{
    ...
    if (pos.check < 0.25) {
        msg <- paste0("please change positions for chromosome ", i, " to build 38 as less than 25% of common controlsnps are found")
        errorlist <- c(errorlist, msg)
        warning("ERROR: ", msg)
    }
}
