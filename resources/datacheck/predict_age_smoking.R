...
hannum_plot <- ggplot(data=pheno, aes(x=Age_numeric,y=Hannum_age)) +
            ...
            labs(title=paste0(study_name," Hannum age vs age, cor =",signif(cor(pheno$Age_numeric,ret$score),digits = 2),"\np =",signif(cor.test(pheno$Age_numeric,ret$score)$p.value),digits=2))+
...
if ("Smoking_factor" %in% colnames(pheno)) {
  ...
  mcigarette_plot <- ggplot(data=pheno, aes(x=Smoking_factor,y=p_smoking_mcigarette,color=Smoking_factor)) +
    ...
    labs(title=paste0(study_name," reported vs predicted smoking (mcigarette), \np=",signif(wilcox.test(p_smoking_mcigarette ~ Smoking_factor, data = pheno, alternative = c("two.sided"))$p.value),digits=2))+
    theme_minimal()+
...
if ("Smoking_factor" %in% colnames(pheno)) {
  ...
  Elliott_plot <- ggplot(data=pheno, aes(x=Smoking_factor,y=p_smoking_elliott,color=Smoking_factor)) +
    geom_boxplot()+
    scale_colour_viridis(discrete=T,begin=0,end=0.65)+
    labs(title=paste0(study_name," reported vs predicted smoking (Elliott), \np=",signif(wilcox.test(p_smoking_elliott ~ Smoking_factor, data = pheno, alternative = c("two.sided"))$p.value),digits=2))+
    theme_minimal()+
    theme(axis.text.x = element_text(angle = 15, vjust = 1, hjust=1))+
    theme(legend.position = "none")
} else if ("Maternal_smoking_factor" %in% colnames(pheno)){
